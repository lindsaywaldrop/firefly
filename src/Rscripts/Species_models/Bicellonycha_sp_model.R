...
#### Parameters ####
parameters <- list(
  Species = "Bicellonycha_sp",          # Species name
  # Measurements from morphology on antennomere 1
  width.olf.hair = 1.89e-6,         # mean width of an olfactory sensillum
  width.mech.hair.prox = 5.32e-6,   # mean width of an mechanosensory sensillum (proximal)
  width.mech.hair.med = 3.60e-6,    # mean width of an mechanosensory sensillum (medial)
  width.mech.hair.dis = 1.30e-6,    # mean width of an mechanosensory sensillum (distal)
  olf.hair.length = 17.0e-6,        # Mean length of an olfactory sensillum
  mech.hair.length = 104.8e-6,       # Mean length of a mechanosensory sensillum
  mech.hair.angle = (29/360)*2*pi,  # Hair angle of a mechanosensory sensillum
  mean.dist.all = 29.62e-6,          # Mean distance between any two sensilla in the array
  sd.dist.all= 7e-6,                # Standard deviation of distances between sensilla
  frac.olf = 0.34,                  # Fraction of all sensilla that are olfactory
  density.all = 0.00217/(1e-6)^2,    # Mean density of all sensilla 
  width.ant = 192.6e-6,               # width of antennomere 6
