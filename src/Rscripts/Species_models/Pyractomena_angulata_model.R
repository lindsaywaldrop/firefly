...
#### Parameters ####
parameters <- list(
  Species = "Pyractomena_angulata",          # Species name
  # Measurements from morphology on antennomere 4
  width.olf.hair = 2.03e-6,         # mean width of an olfactory sensillum
  width.mech.hair.prox = 3.69e-6,   # mean width of an mechanosensory sensillum (proximal)
  width.mech.hair.med = 1.60e-6,    # mean width of an mechanosensory sensillum (medial)
  width.mech.hair.dis = 0.74e-6,    # mean width of an mechanosensory sensillum (distal)
  olf.hair.length = 13.9e-6,        # Mean length of an olfactory sensillum
  mech.hair.length = 52.8e-6,       # Mean length of a mechanosensory sensillum
  mech.hair.angle = (29/360)*2*pi,  # Hair angle of a mechanosensory sensillum
  mean.dist.all = 71.12e-6,          # Mean distance between any two sensilla in the array
  sd.dist.all= 14e-6,                # Standard deviation of distances between sensilla
  frac.olf = 0.21,                  # Fraction of all sensilla that are olfactory
  density.all = 0.00041/(1e-6)^2,    # Mean density of all sensilla 
  width.ant = 383e-6,               # width of antennomere 6
