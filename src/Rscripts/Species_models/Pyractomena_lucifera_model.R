...
n <- 101     # set seed number
reps <- 50  # number of replicates

#### Parameters ####
parameters <- list(
  Species = "Pyractomena_lucifera",          # Species name
  ...
  width.mech.hair.med = 1.92e-6,    # mean width of an mechanosensory sensillum (medial)
  width.mech.hair.dis = 0.67e-6,    # mean width of an mechanosensory sensillum (distal)
  olf.hair.length = 12.3e-6,        # Mean length of an olfactory sensillum
  mech.hair.length = 54.1e-6,       # Mean length of a mechanosensory sensillum
  mech.hair.angle = (29/360)*2*pi,  # Hair angle of a mechanosensory sensillum
  mean.dist.all = 23.7e-6,          # Mean distance between any two sensilla in the array
  sd.dist.all= 7e-6,                # Standard deviation of distances between sensilla
  frac.olf = 0.35,                  # Fraction of all sensilla that are olfactory
  density.all = 0.0036/(1e-6)^2,    # Mean density of all sensilla 
  width.ant = 131.5e-6,               # width of antennomere 6
