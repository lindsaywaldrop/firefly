...
#### Parameters ####
parameters <- list(
  Species = "Lucidota_punctata",        # Species name
  # Measurements from morphology on antennomere 6
  width.olf.hair = 8.31e-6,         # mean width of an olfactory sensillum
  width.mech.hair.prox = 3.65e-6,   # mean width of an mechanosensory sensillum (proximal)
  width.mech.hair.med = 2.03e-6,    # mean width of an mechanosensory sensillum (medial)
  width.mech.hair.dis = 1.07e-6,    # mean width of an mechanosensory sensillum (distal)
  olf.hair.length = 11.0e-6,        # Mean length of an olfactory sensillum
  mech.hair.length = 66.1e-6,       # Mean length of a mechanosensory sensillum
  mech.hair.angle = (29/360)*2*pi,  # Hair angle of a mechanosensory sensillum
  mean.dist.all = 27.4e-6,          # Mean distance between any two sensilla in the array
  sd.dist.all= 7e-6,                # Standard deviation of distances between sensilla
  frac.olf = 0.55,                  # Fraction of all sensilla that are olfactory
  density.all = 0.0026/(1e-6)^2,    # Mean density of all sensilla 
  width.ant = 292e-6,               # width of antennomere 6
