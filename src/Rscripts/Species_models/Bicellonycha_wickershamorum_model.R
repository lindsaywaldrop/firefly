...
n <- 144     # set seed number
reps <- 10  # number of replicates

#### Parameters ####
parameters <- list(
  Species = "Bicellonycha_wickershamorum",          # Species name
  # Measurements from morphology on antennomere 6
  width.olf.hair = 1.95e-6,         # mean width of an olfactory sensillum
  width.mech.hair.prox = 4.18e-6,   # mean width of an mechanosensory sensillum (proximal)
  width.mech.hair.med = 3.01e-6,    # mean width of an mechanosensory sensillum (medial)
  width.mech.hair.dis = 0.91e-6,    # mean width of an mechanosensory sensillum (distal)
  olf.hair.length = 13.7e-6,        # Mean length of an olfactory sensillum
  mech.hair.length = 82.8e-6,       # Mean length of a mechanosensory sensillum
  mech.hair.angle = (29/360)*2*pi,  # Hair angle of a mechanosensory sensillum
  mean.dist.all = 25.2e-6,          # Mean distance between any two sensilla in the array
  sd.dist.all= 7e-6,                # Standard deviation of distances between sensilla
  frac.olf = 0.11,                  # Fraction of all sensilla that are olfactory
  density.all = 0.0031/(1e-6)^2,    # Mean density of all sensilla 
  width.ant = 95.4e-6,               # width of antennomere 6
