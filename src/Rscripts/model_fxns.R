...
make.model <- function(parameters, rep, plotit = FALSE){
  ...
  if(plotit==TRUE){
    ...
    points(x = positions[hairs == 1], rep(0,length(positions[hairs == 1])), pch = 19, col = "red")
    lines(c(-200e-6, 200e-6), c(0, 0))
  }
  
  # Make fixed pegs
  length.peg <- ifelse(hairs == 1, parameters$olf.hair.length, parameters$mech.hair.length*0.10)
  width.peg <- ifelse(hairs == 1,  parameters$width.olf.hair, parameters$width.mech.hair.prox)
  peg.ids <- data.frame("start_id" = rep(NA, parameters$num.olf.hairs), "numPts" = rep(NA, parameters$num.olf.hairs))
  pegs <- data.frame()
  count.hairs <- 0
  for(k in 1:2){
  ...
  write.vertex(parameters$Species, pegs, rep)
  write.pegids(parameters$Species, peg.ids, rep)
